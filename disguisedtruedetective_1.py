# -*- coding: utf-8 -*-
"""DisguisedTrueDetective_1.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1-Tl_54Ow979Wh42BMP6lKVqaeWzUVrGO
"""
...
Y_Set2 = []
for i in range(692): #X_train2.shape[0]
  if i>= 0 and i<48:
...
Y_Set = []
for i in range(497): #X_test.shape[0]
  if i>= 0 and i<54:
    Y_Set.append(9) #laiba
  elif i>= 54 and i < 107:
    Y_Set.append(3) #saad
...
faceData = np.load('/content/gdrive/MyDrive/FYP (2020-2021)/DataSets/FYP-1-Files/True-Detective-Aligned-faces-dataset.npz')
...
#for unaligned
#Loading the face dataset.
faceData2 = np.load('/content/gdrive/MyDrive/FYP (2020-2021)/DataSets/FYP-1-Files/True-Detective-un-Aligned-faces-dataset.npz')
...
**(a)** EPOCHS = 100 & BATCH_SIZE = 32
"""

epochs = 100 # epochs = # of iterations over the entire dataset.
batch_size = 32
...
"""**(b)** EPOCHS = 30 & BATCH_SIZE = 32"""

epochs = 30 # epochs = # of iterations over the entire dataset.
batch_size = 32

# early stopping is a form of regularization used to avoid overfitting when training a learner with an iterative method.
checkpointer = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=20)
...
model.save_weights('dcnn1_weights.h5')

model.save("dcnn_1.h5")

model = load_model('dcnn_1.h5')
# summarize model.
model.summary()
...
**(a)** EPOCH = 100 & batch size = 32.
"""

epochs = 100 # for better result increase the epochs. epochs = # of iterations over the entire dataset.
batch_size = 32
...
"""**(b)** EPOCH = 23 & batch size = 32."""

epochs = 23 # for better result increase the epochs. epochs = # of iterations over the entire dataset.
batch_size = 32
...
model2.save_weights('dcnn2_weights.h5')

model2.save('dcnn_2.h5')

model2 = load_model('dcnn_2.h5')
# summarize model.
model2.summary()
...
"""**(4.5) EXTRACT UN-ALIGNED FEATURE.**"""
...
Train_Feats = np.concatenate([Xtrain_feats1, Xtrain_feats2], 3)

print(Train_Feats.shape)
print(Train_Feats[0])
print(Train_Feats[0].shape)

"""**(5.2) X-TEST FEATURES PREDICT & CONCATENATE**"""
...
Test_Feats = np.concatenate([Test_Feat1, Test_Feat2], 3)#3

print(Test_Feats.shape)
print(Test_Feats[0])
print(Test_Feats[0].shape)

"""**(5.3) SAVE & LOAD TRAIN & TEST FEATURES**"""

#for unaligned
np.savez_compressed('cnn-trainFeat-testFeat.npz', Train_Feats, Test_Feats)

#Loading the face dataset.
faceData = np.load('cnn-trainFeat-testFeat.npz')
...
from sklearn.model_selection import train_test_split
x_train, x_val, y_train, y_val = train_test_split(Xtrain_feats1, Y_Train, test_size = 0.1, random_state = 2)
print("x_train shape: ",x_train.shape)
print("x_val shape: ",x_val.shape)
print("y_train shape: ",y_train.shape)
print("y_val shape :",y_val.shape)
...
dimension as the input. "causal" results in causal (dilated) convolutions
'''
cnn_model.add(Conv2D(filters = 64, kernel_size = (3,3),strides=1, padding = 'Same', input_shape=(12, 12, 256))) #(28, 28, 3)))
...
#2. LAYER - Conv layer  CONV2
cnn_model.add(Conv2D(filters = 128, kernel_size = (3,3), strides=1,padding = 'Same'))
cnn_model.add(BatchNormalization())
cnn_model.add(Activation("relu"))
...
#3. LAYER - Conv layer  CONV3
cnn_model.add(Conv2D(filters = 256, kernel_size = (3,3),strides=1, padding = 'Same'))
cnn_model.add(BatchNormalization())
cnn_model.add(Activation("relu"))

#4. LAYER - Conv layer  CONV4
cnn_model.add(Conv2D(filters = 256, kernel_size = (3,3),strides=1, padding = 'Same'))
cnn_model.add(BatchNormalization())
cnn_model.add(Activation("relu"))
...
#FULLY CONNECTED LAYER FC1: fully connected layer with 256 inputs, 1024 outputs
...
#FULLY CONNECTED LAYER  FC2: fully connected layer with 1024 inputs, 1024 outputs
...
#OUTPUT LAYER - SOFTMAX: softmax layer for classification: 1024 inputs, 10 outputs
cnn_model.add(Dense(10, activation='softmax')) #The output Softmax layer has 10 nodes, one for each class.
...
"""**(b)** Epochs = 76 & batchsize = 32."""

epochs = 76 # for better result increase the epochs. epochs = # of iterations over the entire dataset.
batch_size = 32
...
loss, acc = cnn_model.evaluate(x_train, y_train, verbose=1)
print("  Training : loss %.3f - acc %.3f" % (loss, acc))

print('Cross-validation data:', flush=True)
loss, acc = cnn_model.evaluate(x_val, y_val, verbose=1)
...
model3 = load_model("cnn1.h5")

"""**ACCURACY TEST SET**"""

score = cnn_model.evaluate(Test_Feat1,Y_Test,verbose=0)
print("Test Loss:",score[0])
print("Test Accuracy:",score[1])
...
yhat_classes = cnn_model.predict_classes(Test_Feat1, verbose=0)
